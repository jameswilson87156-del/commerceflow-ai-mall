...
The following CI run exposed an independent test-order dependency: Spring test classes shared the same H2 database and order tests could leave changed inventory and orders for later AI and overview assertions. Test-only `BeforeEach` setup now restores the documented Showcase seed facts before those assertions. It changes no production transaction, schema, fixture migration, or runtime behavior.
