...
public class CatalogRepository {
    ...
    public List<ApiModels.ProductSummary> findProducts() {
        return jdbc.query("SELECT p.id,p.name,p.description,c.name category_name FROM product p JOIN product_category c ON c.id=p.category_id WHERE p.status='ON_SALE' ORDER BY p.id", (rs, n) -> product(rs.getLong("id")));
    }
    public Optional<ApiModels.ProductSummary> findProduct(long id) {
        return jdbc.query("SELECT p.id,p.name,p.description,c.name category_name FROM product p JOIN product_category c ON c.id=p.category_id WHERE p.id=?", (rs, n) -> product(rs.getLong("id")), id).stream().findFirst();
    }
    private ApiModels.ProductSummary product(long id) {
        var row = jdbc.queryForMap("SELECT p.id,p.name,p.description,c.name category_name FROM product p JOIN product_category c ON c.id=p.category_id WHERE p.id=?", id);
        var skus = jdbc.query("SELECT s.id,s.sku_code,s.color,s.size,s.sale_price,s.currency,i.available_stock FROM product_sku s JOIN inventory i ON i.sku_id=s.id WHERE s.product_id=? AND s.status='ON_SALE' ORDER BY s.id", (rs, n) -> new ApiModels.Sku(rs.getLong("id"), rs.getString("sku_code"), rs.getString("color"), rs.getString("size"), rs.getBigDecimal("sale_price"), rs.getString("currency"), rs.getInt("available_stock")), id);
        return new ApiModels.ProductSummary(((Number) row.get("id")).longValue(), (String) row.get("name"), (String) row.get("description"), (String) row.get("category_name"), skus);
    }
    public ApiModels.Sku sku(long skuId) {
        return jdbc.queryForObject("SELECT s.id,s.sku_code,s.color,s.size,s.sale_price,s.currency,i.available_stock FROM product_sku s JOIN inventory i ON i.sku_id=s.id WHERE s.id=? AND s.status='ON_SALE'", (rs, n) -> new ApiModels.Sku(rs.getLong("id"), rs.getString("sku_code"), rs.getString("color"), rs.getString("size"), rs.getBigDecimal("sale_price"), rs.getString("currency"), rs.getInt("available_stock")), skuId);
    }
