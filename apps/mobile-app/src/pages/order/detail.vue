...
<template>
  <view class="mobile-page order-detail-page">
    ...
    <template v-else-if="order"><view class="order-header panel"><view><text class="eyebrow">ORDER DETAIL</text><text class="order-no">{{ order.orderNo }}</text></view><text class="tag tag-success">已创建</text><text class="muted">CREATED · {{ order.createdAt }}</text></view><view class="section-title"><text>订单商品快照</text><text class="muted">历史下单时保存</text></view><view v-for="(item,index) in order.items" :key="`${item.skuCodeSnapshot}-${index}`" class="order-item panel"><ProductImage class="order-image" :src="imageUrl(item)" :alt="item.productNameSnapshot" placeholder="暂无历史图片" /><view class="order-copy"><text class="cart-name">{{ item.productNameSnapshot }}</text><text class="sku-code">{{ item.skuCodeSnapshot }}</text><text class="cart-spec">{{ item.colorSnapshot }} / {{ item.sizeSnapshot }} · 数量 {{ item.quantity }}</text><text class="cart-price">¥{{ item.unitPrice }} {{ order.currency }}</text></view><text class="line-total">¥{{ lineTotal(item) }}</text></view><view class="order-total panel"><text class="muted">订单总额</text><text class="total-value">¥{{ order.totalAmount }} <text class="currency">{{ order.currency }}</text></text></view></template>
  </view>
</template>

<style>
.order-detail-page{padding-bottom:70px}.order-header{display:flex;align-items:center;gap:14px;flex-wrap:wrap;padding:20px}.order-header .muted{width:100%}.order-no{display:block;margin-top:7px;font-family:monospace;font-size:25px;font-weight:800}.order-item{display:flex;align-items:center;gap:16px;padding:16px;margin-bottom:14px}.order-image{width:112px;height:112px;flex:none;border-radius:12px}.order-copy{min-width:0;flex:1}.order-total{display:flex;align-items:flex-end;justify-content:space-between;padding:18px 20px;margin-top:18px}
</style>
