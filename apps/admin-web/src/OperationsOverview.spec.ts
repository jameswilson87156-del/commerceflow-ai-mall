...
describe('OperationsOverview', () => {
  ...
  it('shows error and retries the real endpoint', async () => {
    ...
    expect(wrapper.get('[data-testid="operations-access-state"]').text()).toBe('Backend Unavailable')
  ...
  it('surfaces unauthorized and forbidden operator states instead of using fixture data', async () => {
    const unauthorized = vi.fn().mockResolvedValueOnce(new Response(JSON.stringify({ code: 'OPERATOR_UNAUTHENTICATED' }), { status: 401 }))
    vi.stubGlobal('fetch', unauthorized)
    const unauthorizedWrapper = mount(OperationsOverview)
    await flushPromises()
    expect(unauthorizedWrapper.get('[data-testid="operations-access-state"]').text()).toBe('Unauthorized')
    expect(unauthorizedWrapper.text()).not.toContain('¥328.00')

    vi.stubGlobal('fetch', vi.fn().mockResolvedValueOnce(new Response(JSON.stringify({ code: 'OPERATOR_FORBIDDEN' }), { status: 403 })))
    const forbiddenWrapper = mount(OperationsOverview)
    await flushPromises()
    expect(forbiddenWrapper.get('[data-testid="operations-access-state"]').text()).toBe('Forbidden')
    expect(forbiddenWrapper.text()).not.toContain('¥328.00')
  })
})
